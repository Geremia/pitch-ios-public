...
class PitchPipeView: UIView {
    ...
    func allOff() {
        for button in self.pitchButtons {
            ...
            Mixer.shared.soundGenerator.playNoteOff(channelNumber: button.tag)
        }
    }
    ...
    @IBAction func pitchButtonPressed(_ sender: AnyObject) {
        ...
        if isActive {
            ...
            Mixer.shared.soundGenerator.playNoteOn(channelNumber: button.tag)
        }
    }
    ...
    @IBAction func minusButtonPressed(_ sender: AnyObject) {
        ...
        if currentOctave > 2 {
            currentOctave -= 1
            Mixer.shared.soundGenerator.decrementOctave()
    ...
    @IBAction func plusButtonPressed(_ sender: AnyObject) {
        ...
        if currentOctave < 7 {
            currentOctave += 1
            Mixer.shared.soundGenerator.incrementOctave()
            octaveLabel.text = String(currentOctave)
        }
    }
}
